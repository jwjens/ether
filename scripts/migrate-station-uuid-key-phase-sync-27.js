'use strict';
// Migration v27 — station UUID re-key (v4.5.0), Phase 0 (ADDITIVE, non-switching).
//
...
// no-op. Verify on a COPY first: `node scripts/migrate-station-uuid-key-phase-sync-27.js <copy.db>`.
...
function applyMigration(db) {
  const migrate = db.transaction(() => {
    ...
    try { db.prepare('INSERT INTO schema_version (version) VALUES (27)').run(); } catch { /* already recorded */ }
    console.log(`[migrate-v27] station_uuid: +${added} columns, backfilled ${backfilled} rows across ${tables.length} scoped tables (${tables.join(', ')})`);
  });
  migrate();
...
if (require.main === module) {
  ...
  console.log('=== migrate-station-uuid-key-phase-sync-27.js ===');
