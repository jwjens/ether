// Migration v14: add station_id to midi_mappings
...
if (require.main === module) {
  ...
  console.log("=== migrate-station-schema-phase-sync-14.js ===");
  console.log("DB:", dbPath);

  // ── Pre-flight 1: schema_version must be 1..13 ────────────────
  ...
  if (currentVersion < 1 || currentVersion > 13) {
    console.error("ABORT: expected schema_version in [1..13], got", currentVersion);
    process.exit(1);
  }

  // ── Pre-flight 2: station_id must not already exist ───────────
  const cols = db.prepare("PRAGMA table_info(midi_mappings)").all().map(r => r.name);
  console.log("midi_mappings columns:", cols);
  ...
  check("schema_version = 14", newVersion === 14, `got ${newVersion}`);

  const newCols = db.prepare("PRAGMA table_info(midi_mappings)").all().map(r => r.name);
  check("station_id column exists", newCols.includes("station_id"), newCols.join(", "));

  const idxRows = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_midi_mappings_station_id'").all();
  check("idx_midi_mappings_station_id exists", idxRows.length > 0);

  const nullCount = db.prepare("SELECT COUNT(*) as c FROM midi_mappings WHERE station_id IS NULL").get().c;
  check("no NULL station_id rows", nullCount === 0, `null count: ${nullCount}`);

  const wrongCount = db.prepare("SELECT COUNT(*) as c FROM midi_mappings WHERE station_id != 1").get().c;
  check("all existing rows have station_id=1", wrongCount === 0, `non-1 count: ${wrongCount}`);

  const totalRows = db.prepare("SELECT COUNT(*) as c FROM midi_mappings").get().c;
  ...
  console.log("\nAll checks PASSED — migration v14 complete.");
}
