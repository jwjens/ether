...
//
// REPORT BEFORE WRITE, ALWAYS. Dry run by default; --write is required, and even then it reports
// first. NEVER RUN AGAINST A LIVE DATABASE WHILE ETHER IS OPEN — close it fully (tray +
// ether-engine) or work on a copy. This script WRITES.
//
...
const { REGISTRY } = require(path.join(__dirname, '..', 'electron', 'sync', 'synced-tables'));
...
rule(`BACKFILL library_asset  —  ${WRITE ? 'WRITE' : 'DRY RUN (nothing will change)'}`);
