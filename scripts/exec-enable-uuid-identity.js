...
const up = db.prepare(`
  ...
  VALUES (?, 'sync_uuid_identity', 'true', ?, ?, ?)
  ON CONFLICT(station_id, key) DO UPDATE SET value='true', updated_at=excluded.updated_at, deleted_at=NULL
`);
const { randomUUID } = require("crypto");
db.transaction(() => { for (const s of stations) up.run(s.id, randomUUID(), now, now); })();
...
const allTrue = rows.length === stations.length && rows.every(r => r.value === "true");
console.log("\nstored:", allTrue ? "true on every station" : "*** MISMATCH ***");
