...
- **DOORS BEFORE ROOMS — a feature without an obvious door does not exist to users; they will conclude Ether doesn't work.** Every user-facing panel must be reachable from the canonical navigation (the hamburger menu), have a help entry, and explain itself in its empty state. "It's in the code" is not shipped. (Proven failures: jingles, Show+ DAW, Spots — each a real room reachable only through a hidden or native-menu-only door, so users concluded the feature was missing.)
