...
// electron/sync/handlers/macros.js — generated by scripts/generate-handlers.js
// IPC handlers for the macros table (station-scoped).
// Channels: macros:list | :get-by-id | :create | :update | :delete
// Preload:  window.ether.macros.*
//
...
function validateScope() {
  ...
  if (!entry) throw new Error(`[macros] unknown table in registry: "${TABLE}"`);
  if (entry.scope !== 'station') {
    throw new Error(`[macros] expected station-scoped table, registry has "${entry.scope}"`);
  }
}

// ── Business logic ────────────────────────────────────────────────────────────

function macrosList(db, stationId, opts) {
...
function macrosGet(db, uuid) {
  return db.prepare(
    `SELECT * FROM ${TABLE} WHERE uuid = ? AND deleted_at IS NULL`
  ).get(uuid) ?? null;
}

function macrosCreate(db, payload) {
  validateScope();
  if (HAS_STATION_ID_COL && payload.station_id == null) {
    throw new Error(`[macros] station_id is required for station-scoped create`);
  }
  ...
  withMutation(db, {
    ...
    db.prepare(
      `INSERT INTO ${TABLE} (name, description, trigger_type, trigger_value, actions, hotkey, is_active, color, created_at, station_id, uuid, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(row.name, row.description, row.trigger_type, row.trigger_value, row.actions, row.hotkey, row.is_active, row.color, row.created_at, row.station_id, row.uuid, row.updated_at, row.deleted_at);
  });
  return macrosGet(db, uuid);
}

function macrosUpdate(db, uuid, patch) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[macros] row not found: ${uuid}`);

  const forbidden = Object.keys(patch).filter(k => k !== 'actor_id' && !PATCHABLE.includes(k));
  if (forbidden.length > 0) {
    throw new Error(`[macros] cannot patch immutable field(s): ${forbidden.join(', ')}`);
  }

  const patchFields = PATCHABLE.filter(k => k in patch);
  if (patchFields.length === 0) {
    throw new Error(`[macros] no patchable fields provided in patch`);
  }
...
function macrosDelete(db, uuid, stationId) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[macros] row not found: ${uuid}`);
