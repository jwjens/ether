...
const PATCHABLE          = ["clock_id","position","slot_type","category_id","song_id","label","duration_min","chain_type","spot_type","spot_category_id","updated_at"];
...
function clockSlotsCreate(db, payload) {
  ...
  withMutation(db, {
    ...
    db.prepare(
      `INSERT INTO ${TABLE} (clock_id, position, slot_type, category_id, song_id, label, duration_min, spot_type, spot_category_id, station_id, uuid, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(row.clock_id, row.position, row.slot_type, row.category_id, row.song_id ?? null, row.label, row.duration_min, row.spot_type ?? null, row.spot_category_id ?? null, row.station_id, row.uuid, row.created_at, row.updated_at, row.deleted_at);
  });
