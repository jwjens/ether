...
// electron/sync/handlers/rtmp_destinations.js — generated by scripts/generate-handlers.js
// IPC handlers for the rtmp_destinations table (station-scoped).
// Channels: rtmp_destinations:list | :get-by-id | :create | :update | :delete
// Preload:  window.ether.rtmpDestinations.*
//
...
function validateScope() {
  ...
  if (!entry) throw new Error(`[rtmp_destinations] unknown table in registry: "${TABLE}"`);
  if (entry.scope !== 'station') {
    throw new Error(`[rtmp_destinations] expected station-scoped table, registry has "${entry.scope}"`);
  }
}

// ── Business logic ────────────────────────────────────────────────────────────

function rtmpDestinationsList(db, stationId, opts) {
...
function rtmpDestinationsGet(db, uuid) {
  return db.prepare(
    `SELECT * FROM ${TABLE} WHERE uuid = ? AND deleted_at IS NULL`
  ).get(uuid) ?? null;
}

function rtmpDestinationsCreate(db, payload) {
  validateScope();
  if (HAS_STATION_ID_COL && payload.station_id == null) {
    throw new Error(`[rtmp_destinations] station_id is required for station-scoped create`);
  }
  ...
  withMutation(db, {
    ...
    db.prepare(
      `INSERT INTO ${TABLE} (name, url, stream_key, is_active, station_id, uuid, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(row.name, row.url, row.stream_key, row.is_active, row.station_id, row.uuid, row.created_at, row.updated_at, row.deleted_at);
  });
  return rtmpDestinationsGet(db, uuid);
}

function rtmpDestinationsUpdate(db, uuid, patch) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[rtmp_destinations] row not found: ${uuid}`);

  const forbidden = Object.keys(patch).filter(k => k !== 'actor_id' && !PATCHABLE.includes(k));
  if (forbidden.length > 0) {
    throw new Error(`[rtmp_destinations] cannot patch immutable field(s): ${forbidden.join(', ')}`);
  }

  const patchFields = PATCHABLE.filter(k => k in patch);
  if (patchFields.length === 0) {
    throw new Error(`[rtmp_destinations] no patchable fields provided in patch`);
  }
...
function rtmpDestinationsDelete(db, uuid, stationId) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[rtmp_destinations] row not found: ${uuid}`);
