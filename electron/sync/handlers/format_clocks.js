...
// electron/sync/handlers/format_clocks.js — generated by scripts/generate-handlers.js
// IPC handlers for the format_clocks table (station-scoped).
// Channels: format_clocks:list | :get-by-id | :create | :update | :delete
// Preload:  window.ether.formatClocks.*
//
...
function validateScope() {
  ...
  if (!entry) throw new Error(`[format_clocks] unknown table in registry: "${TABLE}"`);
  if (entry.scope !== 'station') {
    throw new Error(`[format_clocks] expected station-scoped table, registry has "${entry.scope}"`);
  }
}

// ── Business logic ────────────────────────────────────────────────────────────

function formatClocksList(db, stationId, opts) {
...
function formatClocksGet(db, uuid) {
  return db.prepare(
    `SELECT * FROM ${TABLE} WHERE uuid = ? AND deleted_at IS NULL`
  ).get(uuid) ?? null;
}

function formatClocksCreate(db, payload) {
  validateScope();
  if (HAS_STATION_ID_COL && payload.station_id == null) {
    throw new Error(`[format_clocks] station_id is required for station-scoped create`);
  }
  ...
  withMutation(db, {
    ...
    db.prepare(
      `INSERT INTO ${TABLE} (name, slots, created_at, daypart, slots_json, station_id, uuid, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(row.name, row.slots, row.created_at, row.daypart, row.slots_json, row.station_id, row.uuid, row.updated_at, row.deleted_at);
  });
  return formatClocksGet(db, uuid);
}

function formatClocksUpdate(db, uuid, patch) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[format_clocks] row not found: ${uuid}`);

  const forbidden = Object.keys(patch).filter(k => k !== 'actor_id' && !PATCHABLE.includes(k));
  if (forbidden.length > 0) {
    throw new Error(`[format_clocks] cannot patch immutable field(s): ${forbidden.join(', ')}`);
  }

  const patchFields = PATCHABLE.filter(k => k in patch);
  if (patchFields.length === 0) {
    throw new Error(`[format_clocks] no patchable fields provided in patch`);
  }
...
function formatClocksDelete(db, uuid, stationId) {
  validateScope();
  const existing = db.prepare(`SELECT * FROM ${TABLE} WHERE uuid = ?`).get(uuid);
  if (!existing) throw new Error(`[format_clocks] row not found: ${uuid}`);
