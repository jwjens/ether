...
//   §12.2    [N-86]/[N-88]  — no deletes from mutations table; compactMutations() is a stub
 ...
 * @param {string|null}   opts.parent_mutation_id causal parent UUID or null
 *
...
/**
 * Stub per [N-88]. Compaction is out of scope for v0.
 *
 * Exposed so callers can write `mutationWriter.compactMutations(db)` today and
 * receive a clear error rather than "function not found". Real implementation
 * is deferred to the sync engine and requires the checkpoint mechanism reserved
 * by [N-10]'s op='checkpoint' value and the retention rules in [N-83]–[N-85].
 *
 * @param {import('better-sqlite3').Database} db  unused in v0
 */
function compactMutations(db) { // eslint-disable-line no-unused-vars
  throw new Error(
    'mutation-writer: compactMutations not implemented in v0; see [N-88]. ' +
    'Will be addressed by sync engine.'
  );
}
