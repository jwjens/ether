...
function runMigrations() {
  ...
  // Stamp schema_version for fresh installs. On an existing install this table
  // already has rows from the standalone migration scripts. On a fresh install it
  // is empty — mutation-writer hard-throws and the pre-commit hook fails.
  // Derive the version from migration filenames (same regex as transformer-chain.js)
  // so adding migrate-*-phase-sync-17.js automatically updates the stamp.
  {
    const svEmpty = !db.prepare("SELECT 1 FROM schema_version LIMIT 1").get();
    if (svEmpty) {
      const migrateRe = /^migrate-.+-phase-sync-(\d+)\.js$/;
      const scriptsDir = path.join(__dirname, '..', 'scripts');
      let maxN = 0;
      try {
        for (const f of require('fs').readdirSync(scriptsDir)) {
          const m = migrateRe.exec(f);
          if (m) maxN = Math.max(maxN, parseInt(m[1], 10));
        }
      } catch (_) {}
      if (maxN > 0) {
        const stmt = db.prepare("INSERT OR IGNORE INTO schema_version (version) VALUES (?)");
        for (let v = 1; v <= maxN; v++) stmt.run(v);
        console.log("[DB] schema_version stamped: 1.." + maxN, "(fresh install)");
      }
    }
  }
