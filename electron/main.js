...
function _surfaceProcTargetDefault() {
  ...
  try {
    ...
    for (const st of stations) {
      ...
      if (existing) continue;                      // the operator's number — never overwrite
      stationConfigKvUpsertByKey(db, st.id, 'proc_target_lufs', EFFECTIVE_DEFAULT);
      written++;
      _healthEvent('proc-target-surfaced', {
        stationId: st.id, station: st.name, value: EFFECTIVE_DEFAULT,
        reason: 'the effective default was never written down; recorded as the current value',
      });
    }
    if (written) console.log(`[processing] surfaced the ${EFFECTIVE_DEFAULT} LUFS target on ${written} station(s)`);
  } catch (e) { console.error('[processing] target surfacing failed (non-fatal):', e.message); }
}
...
function startAutoExtend() {
  ...
  try { _surfaceProcTargetDefault(); } catch {}     // record the -14 loudness target as a visible value
