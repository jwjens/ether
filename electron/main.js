...
//
// PHASE A GATES NOTHING. _autoExtendTick still runs on every switched-on machine.
...
function _kvPut(stationId, key, value) {
  ...
  try {
    for (const r of db.prepare("SELECT station_id, value FROM station_config_kv WHERE key='kill_lease'").all()) {
      if (String(r.value) === '1' && !_kvGet(r.station_id, 'kill_designation')) {
        _kvPut(r.station_id, 'kill_designation', '1');
...
function _designationTick(generatedFor) {
  ...
  for (const st of stations) {
    ...
    if (d.action === 'designate' || d.action === 'stamp') {
      try {
        _kvPut(st.id, _desig.KEY, _desig.nextRecord({ record, now, machineId: me.id, machineName: me.name,
          generated: !!(generatedFor && generatedFor.has(st.id)) }));
      } catch (e) { console.error('[designation] write failed:', e.message); }
    }
...
function startAutoExtend() {
  ...
  try { _designationTick(new Set()); } catch (e) { console.error('[designation] first tick:', e.message); }
