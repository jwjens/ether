...
app.whenReady().then(() => {
  ...
  // ONE-TIME DB JOBS — here, after initDb(), because `db` does not exist at module load. Both of these
  // were called from startAutoExtend() at module scope and silently did nothing (see the note there).
  try { _migrateAutoGenerateDefault(); } catch (e) { console.error('[auto-extend] migration call failed:', e.message); }
  try { _surfaceProcTargetDefault(); } catch (e) { console.error('[processing] target surfacing call failed:', e.message); }
...
ipcMain.handle("audio:set-slot-kind", (_, stationId, deck, kind) =>
  AUDIO_DAEMON ? audiodClient.cmd("setSlotKind", { stationId, deck, kind })
               : audio.audioSetSlotKind(stationId, deck, String(kind || "")));
...
// THE STATION'S LOUDNESS TARGET HAS ALWAYS BEEN -14, AND IT HAS NEVER BEEN WRITTEN DOWN. It lives as a
// literal in three places — audiod/engine.js (the KV read's own fallback), native/src/audio.rs (bus init)
// and the ProgramProcessor's construction — so a station with no row runs -14 and nothing on screen says
// so. This records the effective value AS the value, once, per station.
//
// IT CHANGES NO SOUND. -14 is exactly what all three paths already deliver, and the mixer calls
// set_target(-14.0) on every buffer today, so there is not even a first-application transient. The write
// makes the number visible; that is its entire effect.
//
// upsertByKey, NOT setLocal: this is a decision about the station's SOUND and must travel with the
// account to any machine the operator signs in on. auto_generate_enabled next door is deliberately
// local-only — the opposite case, and the reason both writers exist.
//
// Idempotent by construction: a station that already has a row is skipped, so this can run on every boot
// forever and needs no done-marker. Non-fatal on error — a station that cannot be surfaced still airs.
function _surfaceProcTargetDefault() {
  const EFFECTIVE_DEFAULT = '-14';   // keep in step with audiod/engine.js and native/src/audio.rs
  try {
    const { stationConfigKvUpsertByKey } = require('./sync/handlers/station_config_kv');
    ...
    for (const st of stations) {
      const existing = db.prepare(
        "SELECT 1 FROM station_config_kv WHERE station_id = ? AND key = 'proc_target_lufs' AND deleted_at IS NULL LIMIT 1"
      ).get(st.id);
      if (existing) { detail.push({ stationId: st.id, station: st.name, action: 'kept' }); continue; }
      stationConfigKvUpsertByKey(db, st.id, 'proc_target_lufs', EFFECTIVE_DEFAULT);
      written++;
      detail.push({ stationId: st.id, station: st.name, action: 'written' });
    }
    // ALWAYS a health event, written or not. The first version of this logged only to a main-process
    // console that does not exist in a packaged app, so "it threw before doing anything" and "there was
    // nothing to do" looked identical from the outside — and the difference took four passes to find.
    // stationsSeen is the tell: 0 means the handle was not ready, not that every station was set.
    _healthEvent('proc-target-surfaced', {
      stationsSeen: stations.length, written, skipped: stations.length - written,
      value: EFFECTIVE_DEFAULT, stations: detail,
      reason: written
        ? 'the effective default was never written down; recorded as the current value'
        : 'every station already had a stored target — nothing to record',
    });
    console.log(`[processing] target ${EFFECTIVE_DEFAULT} LUFS — ${stations.length} station(s) seen, ${written} written`);
  } catch (e) {
    console.error('[processing] target surfacing FAILED:', e.message);
    try { _healthEvent('proc-target-surface-failed', { error: e.message }); } catch {}
  }
}
...
function _migrateAutoGenerateDefault() {
  try {
    ...
    // A COMPLETION EVENT, not just a console line. This migration threw on every launch for weeks —
    // called at module scope where `db` is undefined — and the only trace was a console that does not
    // exist in a packaged app. The marker being absent was the sole evidence, and nothing surfaced it.
    _healthEvent('auto-extend-migration-complete', {
      stationsSeen: stations.length, switchedOn,
      reason: 'one-time carry-across of pre-4.4.185 behaviour; marker set, will not run again',
    });
    ...
    console.error('[auto-extend] default migration FAILED:', e.message);
    try { _healthEvent('auto-extend-migration-failed', { error: e.message }); } catch {}
  }
}
...
function startAutoExtend() {
  ...
  // THE TWO ONE-TIME DB JOBS THAT USED TO RUN HERE NOW RUN IN app.whenReady(), AFTER initDb().
  //
  // startAutoExtend() is called at MODULE SCOPE (bottom of this file), during require() — and `db` is
  // assigned by initDb(), which runs later inside app.whenReady(). So both jobs called db.prepare() on
  // an undefined handle, threw, and were swallowed by their own catch. _migrateAutoGenerateDefault has
  // been failing this way since it was written: its marker (auto_generate_default_migrated) has never
  // been set on this install, so it retried and failed on every single launch.
  //
  // This is the third time this exact trap has been hit in this file — see the GPIO / Site Replication
  // note and the artwork-cache note, both of which say the same thing: db is undefined at module load.
  // The timer below is fine here; only the db-touching work had to move.
