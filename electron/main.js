...
if (AUDIO_DAEMON_DESIRED) {
  ...
  // Phase 1 lease view, per station. Kept out of library-health because the lease is arbitration
  // state, not a library measurement — and because Phase 2 will make it a decision input.
  try { ipcMain.handle("lease:status", () => { try { return [..._leaseStatus.values()]; } catch { return []; } }); } catch {}
...
// ── GENERATION LEASE — Phase 1: OBSERVE ONLY ───────────────────────────────────────────────────
// Runs the arbitration and reports it. It does NOT gate _autoExtendTick: every machine with
// auto-generation ON still generates exactly as before. Phase 2 flips that, once this has been
// watched against two real machines.
const _leaseCore = require('./generation-lease');
let _leaseHolderSeen = new Map();      // stationId -> last holder reported, for transition-only events
let _leaseStatus = new Map();          // stationId -> status object for the Health Monitor
...
function _kvGet(stationId, key) {
  try { const r = db.prepare("SELECT value FROM station_config_kv WHERE station_id=? AND key=? LIMIT 1").get(stationId, key); return r ? r.value : null; }
  catch { return null; }
}

function _leaseTick() {
  const me = _machineIdentity();
  if (!me.id) return;                                  // no stable identity yet — observe nothing
  const now = Math.floor(Date.now() / 1000);
  let stations = [];
  try { stations = db.prepare("SELECT id, name FROM stations WHERE deleted_at IS NULL").all(); } catch { return; }
  for (const st of stations) {
    const kill = String(_kvGet(st.id, 'kill_lease') || '') === '1';
    const autoOn = _autoGenerateEnabled(st.id);
    const lease = _leaseCore.parseLease(_kvGet(st.id, _leaseCore.LEASE_KEY(st.id)));
    const d = _leaseCore.decide({ lease, now, machineId: me.id, autoOn, killSwitch: kill });

    if (d.action === 'claim' || d.action === 'renew') {
      try {
        const { stationConfigKvSetLocal } = require('./sync/handlers/station_config_kv');
        // NOTE: the lease key is SYNCED — it must be, since a local record cannot arbitrate between
        // machines that cannot see each other. It is written through the ordinary create/update path
        // in Phase 2; Phase 1 writes it locally so a half-built feature cannot start syncing claims.
        void stationConfigKvSetLocal;
        db.prepare(`INSERT INTO station_config_kv (station_id, key, value, created_at, updated_at)
          ...
          .run(st.id, _leaseCore.LEASE_KEY(st.id), _leaseCore.nextLease({ lease, now, machineId: me.id, machineName: me.name, claiming: d.action === 'claim' }));
      } catch (e) { console.error('[lease] write failed:', e.message); }
    }

    const after = _leaseCore.parseLease(_kvGet(st.id, _leaseCore.LEASE_KEY(st.id)));
    const stat = _leaseCore.status({ lease: after, now, machineId: me.id, autoOn, killSwitch: kill });
    _leaseStatus.set(st.id, { ...stat, stationId: st.id, station: st.name, machineId: me.id, at: now });

    // TRANSITION ONLY. A 5-minute heartbeat writing every tick would put 288 identical rows a day in
    // the ledger; the live state belongs on screen, and only the CHANGE belongs in the record.
    const holder = after ? after.machine_id : null;
    const prev = _leaseHolderSeen.get(st.id);
    if (prev !== undefined && prev !== holder) {
      _healthEvent('station-lease-changed', { stationId: st.id, station: st.name,
        from: prev, to: holder, initiatedBy: me.id, reason: d.reason, action: d.action });
      console.log(`[lease] ${st.name}: holder ${prev || 'none'} -> ${holder || 'none'} (${d.reason})`);
    }
    _leaseHolderSeen.set(st.id, holder);
  }
}
function _leaseSnapshot(stationId) { return _leaseStatus.get(stationId) || null; }
...
function startAutoExtend() {
  ...
  // Lease heartbeat: observe-only in Phase 1, on its own cadence (5 min) rather than the extender's 30.
  try { _leaseTick(); } catch (e) { console.error('[lease] first tick:', e.message); }
  setInterval(() => { try { _leaseTick(); } catch (e) { console.error('[lease] tick:', e.message); } }, _leaseCore.HEARTBEAT_SEC * 1000);
