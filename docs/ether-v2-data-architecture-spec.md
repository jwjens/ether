...
**Status:** In build — week 1 done; Phase 1 (station_attachments) deployed. See `docs/v2-progress.md`.
**HARD DEADLINE — MONDAY 2026-07-06 is OV live day (on jensj).** Flow: OV account + 3 stations (OV/DRIVE, HalloVeen, Magical Forest) created + categories/schedules authored this week (on this dev box, signed in as OV) → Monday wipe jensj → fresh install → sign in as OV → onboarding placement question offers the 3 stations → check all 3 → scoped libraries land → on air. Monday-critical path: Phases 2–4 (provisioning + placement question), read-cutover, OV library published under OV's own license. See the Monday assessment in `v2-progress.md` for what is Monday-critical vs launch-blocking-but-not-Monday-critical.
...
- **Slice = category membership (locked).** A station's **category/programming assignments ARE its library slice** — ONE membership definition, consumed by BOTH the scheduler and the scoped bootstrap, never two systems. **Audio follows category membership strictly** (fail-closed: a song not in any of the station's categories is not in its audio slice). **Metadata may scope wider than audio** — operators need a picker to assign songs *from* a broader set — but audio delivery never exceeds category membership. Initial seeding may bootstrap categories from `source_folder`, then it's category membership from there on.
- **FLAT LIBRARY (locked).** One source folder going forward; **categories are the ONLY membership mechanism.** The four folders were the ghost-file origin (organization and membership fused → forced duplicate physical copies). `source_folder` is **historical metadata only, never a live scope**; future imports take a flat folder. **No physical reorganization of the existing four folders before Monday** — identity is the hash, the content store is what plays, the folders are archival originals with **zero runtime dependency** (verified 2026-07-02: nothing in `electron/`+`src/` reads the four folders; playback is content-store-by-hash → R2; only the dev importer scans folders).
...
- **OV = its own pool, NO cross-account grants (locked).** Categories authored under the OV account this week ARE OV's import/scoping. OV never reads djdeniro's library (no grants) → the OV structural-isolation test is satisfied by **account/license separation**, independent of intra-account station slicing.
- **R2 same-hash across licenses = STORED TWICE (verified 2026-07-02).** Audio R2 keys are license-namespaced (`<license_id>/<hash>.<ext>`); dedup happens only WITHIN a license. So an object identical by hash in two accounts is stored once per account — physical OV↔djdeniro isolation, not a dedupe. (Storage cost of duplication accepted for isolation.)
...
**STATE TABLE = the MONDAY GATE (not a side deliverable).** A sign-in/onboarding state table: every reachable state, what the user sees, and ≥1 path forward from each. No undefined branches, no generic failure screens, no dead ends. "Nothing to sync yet" is a NORMAL state with a normal screen — the old "Sync failed" dead-end was an *unenumerated* state getting a generic error, so **the cure is enumeration, not better error copy.**
- **Every state this week's work touches gets a row AND a demonstration on this machine where possible:** account switching djdeniro ↔ OV (repeatedly), authoring under OV while this box holds djdeniro's playout claim, the three-station placement answer, flaky-network sign-in, sign-in mid-deploy.
- Rows only demonstrable on jensj are flagged **"Monday-verified"** — nothing pretends to be tested that wasn't. **Undefined state found this week = a bug with time to fix; undefined state found Monday = a failure.**
- **Account switching is a first-class demonstrated feature, not a hazard** — the switching rows are walked on this machine, proving: sessions never bleed, claims/attachments stay per-account, scoped libraries swap cleanly, the badge always tells the truth about who's signed in.
