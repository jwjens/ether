...
- **Phase 3 — Analytics (in progress).** *3a (✅ shipped, v4.2.14):* **playout analytics** — the install pushes its play history up (append-only, incremental), the backend aggregates server-side, and the dashboard shows per-station plays / hours aired / top songs & artists / category mix / daily trend / recent playout log over a date range. *3b (next):* listener metrics (concurrent / tune-in / churn) — needs new capture (listener PWA → backend heartbeat); none exists yet.
- **Phase 4 — Billing/subscription UI.** Later.
