...
**No single active arc — next item is operator choice.** The **Out-of-Process Audio Engine (Item 10)** just shipped (✅ COMPLETE, v4.3.22 + v4.3.23, 2026-05-30/31) — the daemon now owns queue/deck state, survives UI restart/crash, and is dead-air-proof via the stall-recovery watchdog. With Item 10 done, the next arc is **operator choice** among the open items: **Iris-as-Platform (Item 4)**, **AirLogger (Item 7)**, **Plugins/OSS/BYO-Cloud (Item 6)**, or **Onboarding & Library Distribution (Item 8)**. No default has been picked.

**Multi-Tenant Control Center (Item 5)** remains in flight in parallel: Phase 1 (auth, dashboard, live station view, remote sign-in) is in production at `app.ether-technologies.com`; Phase 2a (station/branding settings) and Phase 2b (remote category editing — the first install-authoritative write-back, ✅ verified bidirectionally on OV 2026-05-26) shipped; Phase 2c (clocks/dayparts) is the next Control Center step whenever it resumes.

Completed since the 2026-05-15 lock: **Sync backend** (Item 1, ✅), **High Availability** (Item 3, ✅), **Listener Platform Tier 1** (Item 9 — branded PWA + per-station now-playing, ✅ live at `listen.ether-technologies.com/<slug>`), and **Out-of-Process Audio Engine** (Item 10, ✅). **OV (Windows) and USPH (macOS) are deployed and in active production** on the v4.3.x line.
...
**Status:** ✅ **COMPLETE — shipped 2026-05-30/31 (customer releases v4.3.22 + v4.3.23).** The full state-coordination arc landed as Stages 0 / 1 / 2a / 2b / 3a / 3b. The daemon (`ether-audiod`) is now the single source of truth for queue + deck state; the renderer is read-only (consumes `onDeck`/`onQueue` as authoritative) and writes only via `queue:*` / `deck:*` intent commands keyed by per-entry `qid`, with `boundQids` protecting deck-bound entries. A stall-recovery watchdog (>1s stall → sub-second recovery, respects `manualCue`, resets the `advanceP` wedge) plus Stage 3a's serialized preload fixed the A→B→C rotation stall and make dead-air impossible by construction. The daemon owns queue/advance/scheduler/play-logging and the ffmpeg→Icecast stream; now-playing stays in the app. Engine fixes require a full app close+reopen to take effect (the daemon does **not** reload on auto-update). **One documented follow-up remains (not blocking):** daemon-respawn auto-resume — on a fresh daemon reconnect, if the station was on air the app must auto-reissue `automationStart` (the watchdog is gated on `_started=false`, so manual `automationStart` over the pipe is the current recovery). See `docs/audio-daemon-phase0.md` and the `project_audio_daemon.md` memory. The original ("clean") path of three options for surviving app updates/restarts without dropping on-air audio.
...
| 2026-05-31 | **Out-of-Process Audio Engine (Item 10) COMPLETE — shipped customer releases v4.3.22 + v4.3.23 (2026-05-30/31).** The full state-coordination arc landed as Stages 0/1/2a/2b/3a/3b: daemon (`ether-audiod`) is now the single source of truth for queue + deck state; renderer is read-only via `onDeck`/`onQueue` and writes only through `queue:*` / `deck:*` intent commands keyed by per-entry `qid`, with `boundQids` protecting deck-bound entries (Stages 0/1/2a/2b). The A→B→C rotation stall is fixed by Stage 3a (serialized preload on `advanceP` + freshened `checkEnd` guards) and Stage 3b (stall-recovery watchdog: >1s stall → sub-second recovery, respects `manualCue`, resets the `advanceP` wedge) — dead air is now impossible by construction. Daemon owns queue/advance/scheduler/play-logging + the ffmpeg→Icecast stream; now-playing stays in the app. **Item 10 was the active audio arc; with it done, the next roadmap item is operator choice (Iris/AirLogger/Plugins/Onboarding).** One documented non-blocking follow-up remains: daemon-respawn auto-resume (reissue `automationStart` on reconnect when the station was on air). Operational gotcha recorded: the daemon does NOT reload on app auto-update — a full close+reopen is required for engine fixes to take effect. |
| 2026-09-02 | **publisherName MUST become an array BEFORE the org-cert switch — one release ahead, not with it.** Windows code signing is Azure Trusted Signing with subject `CN=Jeffrey Jens`, and `electron-builder.json` sets `win.publisherName` as a plain string. electron-updater verifies every downloaded update against that value, so the day the certificate changes to an organisation cert with a different CN, existing installs will REJECT the update — silently, as a failed signature check, not a visible error. Nobody gets the fix that would repair it, because the repair arrives inside the update they refuse. The fix is to make it an ARRAY carrying BOTH names (`["Jeffrey Jens", "<org CN>"]`) and ship that in a release signed by the CURRENT cert. Installs then accept the next release under either name, and the old name can be dropped a release later. NOT RETROFITTABLE: once the cert has switched, the array can only travel in an update those installs already refuse. Blocked only on knowing the org cert's exact CN — the string must match it character for character. Same trap, different form, as the v4.2.7 "Latest" badge that silently blocked auto-update. |
