...
- **Phase 2 — Remote editing (in progress).** Two halves: a **read path** (the install pushes its tables to a generic backend mirror so the dashboard can view install-owned data) and a **write path** (the dashboard issues commands over the SSE command bus; the install applies them through its existing sync handlers, producing normal HLC mutations, then re-pushes).
  ...
  - **2c — Clocks / dayparts (in progress).** *2c-1 (✅ shipped, v4.2.9):* the dashboard now views every daypart (with its day/hour range and assigned clock) and every clock (with its slot breakdown), and admins can **reassign which clock a daypart plays** remotely. *2c-2 (next):* slot-level clock editing — add / reorder / remove slots, durations, create clocks.
  - **2d+ — Song library.** The heaviest (audio via the existing R2 signed-URL model; the per-table push must switch to bulk INSERT at library scale — OV is ~5,600 songs). Full functional parity with the desktop is the goal (Jeff, 2026-05-25); same channel, domain by domain.
