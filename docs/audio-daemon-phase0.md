...
- **Sequencing:** Steps 0–5 are ✅ **DONE** (runtime gate; main forwarders; renderer proxy + App.tsx wiring; play-logging; streaming — all behind `ETHER_AUDIO_DAEMON`, default off). **Remaining: Step 6** (lifecycle — the app already spawns the daemon detached via `audio-daemon-client.ensure()`; still TODO = extend the HA watchdog to supervise/restart the daemon, and `asarUnpack` `audiod/**` + `native/ether-audio.node` for packaged builds) and **Step 7** (now-playing push stays in the app — already true, no work). Then the **off-air ACCEPT run** (launch `ETHER_AUDIO_DAEMON=1`, verify everything flows through the daemon, kill the UI → audio + stream + logging continue) before shipping to OV.
