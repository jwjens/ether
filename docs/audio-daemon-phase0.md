...
- **Sequencing:** Steps 0–7 are ✅ **DONE** (runtime gate; main forwarders; renderer proxy + App.tsx wiring; play-logging; streaming; lifecycle/watchdog supervision + packaging; now-playing stays in the app) — all behind `ETHER_AUDIO_DAEMON`, default off.
- **Off-air ACCEPT — daemon core ✅ DONE** (`audiod/accept-offair.js`, run under the real packaged `Ether.exe` via `ELECTRON_RUN_AS_NODE`, temp-DB copy, station 99): **9/9 checks** — playout starts in the daemon, VU levels flow, playstart delivered, queue present, **skip advances**, startStream spawns the encoder off the bus, and the headline — **after the app/UI connection is killed the daemon stays alive and a fresh client sees the deck STILL PLAYING**, with the play log written headlessly. This proves the gapless-restart guarantee end-to-end on the shipping runtime.
- **Remaining = human-eyes only:** launch the real GUI (`ETHER_AUDIO_DAEMON=1; npm run electron:dev`) and visually confirm VU rendering + now-playing display (the React↔IPC↔daemon round-trip the harness exercises directly). Then flip the default on (rollback `=0`) to ship to OV. (Deferred niceties: daemon versioned separately; idle self-exit for unattended stations.)
