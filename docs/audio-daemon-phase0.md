...
### Step 2 — Renderer engine → remote proxy — 🟡 FOUNDATION DONE; cutover wiring + validation pending
Done (`src/audio/engine-rodio.ts`, `electron/main.js`, `electron/preload.js`):
...
- `engine-rodio` gains a `daemonDriven` mode (queried once at init via `daemonEnabled()`): local **advance/preload/refill/jump are disabled** (the poll still reads forwarded `getState` for deck-card display, so it can't race the daemon); it **mirrors** the daemon's queue (`onQueue` → `getQueue()` for the Up Next UI) and **relays** `playstart` (→ `notifyPlayStart`, so App.tsx's now-playing push + play log keep firing); `addToQueue`/`clearQueue`/`replaceQueue` forward to the daemon; `startDaemonAutomation()` added for go-on-air. Typechecks clean.
- **Remaining (needs the running app, off-air):** (1) App.tsx calls `engine.startDaemonAutomation()` at its go-on-air point instead of starting playback locally; (2) suppress the renderer's `loggen` fill when `daemonDriven` (today it would also enqueue → double-fill, since the daemon self-refills via `continuous`); (3) map skip / show-clock `jumpToNextSong` to a daemon command (a daemon `skip` command is a small add). (4) cart load/play already route via the Step-1 `audio:*` forwarders. **Validate** by launching with `ETHER_AUDIO_DAEMON=1` on a non-broadcast machine / station 99 and confirming playout + advance + queue + VU all run through the daemon, then kill the UI and confirm audio continues.
...
### Step 4 — Play-logging → daemon (read-WRITE `node:sqlite`) — spike ✅ DONE
...
- **Spike result** (`scripts/spike-write-contention.js` + `scripts/spike-write-bsq.js`): two processes / two bindings hammered a **copy** of `openair.db` concurrently — `node:sqlite` read-write (daemon role) and `better-sqlite3` under `ELECTRON_RUN_AS_NODE` (app role), **2000 inserts each (4000 total)**. Result: **0 busy errors, 0 hard errors, `integrity_check: ok`, fully cross-visible** (node:sqlite read all of better-sqlite3's rows). WAL + `busy_timeout=5000` makes a contending writer wait microseconds for the lock instead of failing; real cadence (one write every few minutes) is effectively zero contention. **The daemon can safely own play-logging.** Safe by construction — ran against a temp copy + a scratch table, real DB untouched.

### Step 5 — Streaming → daemon (the gapless-stream win) — spike ✅ DONE
...
- **Spike result** (`scripts/spike-ffmpeg-from-programbus.js`, gated `--i-am-off-air`, station 99): the daemon's runtime played a file, served the program bus on a TCP port, and `ffmpeg-static` (spawned as a child) connected, read the f32le PCM, and encoded **97 KB of valid mp3 (frame sync confirmed) at ~128.8 kbit/s** to a local file. Only unproven piece left is the final Icecast network hop — an output-URL swap that's already proven live in the app today. **Note:** stop ffmpeg gracefully (`-t`, or `q` on stdin) — a hard `proc.kill()` on Windows leaves the output unflushed.
...
- **Sequencing:** Step 0 gates everything — ✅ cleared. **All three de-risk spikes (0 runtime, 4 DB-write contention, 5 ffmpeg-in-daemon) are now green**, so Phase 2 has no remaining unknowns — it's implementation. Steps 1–2 are the bulk of the rewiring (no live-audio risk if exercised on station 99 first); Steps 4–5 port proven mechanisms; Steps 6–7 are small. Ship to OV only after the off-air acceptance test passes.
