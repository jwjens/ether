...
3. ~~Move queue + advance + loggen~~ ✅ **DONE** — the daemon now owns the queue, the A→B→C rotate/preload/end-detection, and the `node:sqlite`-backed scheduler:
   ...
   - **Schema finding (important):** `songs` has **no `station_id` column** — it's a single shared library; stations differentiate via clocks/shows/schedule (which are station-scoped). `loggen.ts`'s `buildBaseConditions` references `s2.station_id`, so its **live-pick paths (P2–P4) throw in production and silently fall through to generated_schedule** (caught by `fillQueueFromSchedule`'s try/catch → returns 0). The daemon port uses correct SQL (no song-level station filter). **Follow-up for the app:** fix or remove `s2.station_id` in `loggen.ts:170-174` so the renderer's live-pick fallback actually works.
4. (Phase 2) Re-point the app's `audio:*` IPC + `engine-rodio` at the daemon over the pipe (the only step that touches live audio — needs off-air testing). The daemon takes over play-logging here.
