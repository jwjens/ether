...
console.log('\n  Ether — Icecast Streaming\n');

// ============================================================
// 1. Add stream settings table
// ============================================================

let client = fs.readFileSync('src/db/client.ts', 'utf8');
if (!client.includes('stream_settings')) {
  ...
  console.log('  UPDATED client.ts (stream_settings table)');
}

// ============================================================
// 2. Create StreamManager component
// ============================================================

fs.mkdirSync('src/components', { recursive: true });
fs.writeFileSync('src/components/StreamManager.tsx', `import { useState, useEffect, useRef } from "react";
...
`, 'utf8');
console.log('  CREATED src/components/StreamManager.tsx');

// ============================================================
// 3. Make engine gainNodes accessible for stream capture
// ============================================================

let eng = fs.readFileSync('src/audio/engine.ts', 'utf8');
if (!eng.includes('getAudioContext')) {
  eng = eng.replace(
  ...
  console.log('  UPDATED engine.ts (exposed AudioContext)');
}

// ============================================================
// 4. Wire into App.tsx - replace Settings with tabbed settings
// ============================================================

let app = fs.readFileSync('src/App.tsx', 'utf8');
if (!app.includes('StreamManager')) {
  app = app.replace(
    'import RulesEditor from "./components/RulesEditor";',
    'import RulesEditor from "./components/RulesEditor";\nimport StreamManager from "./components/StreamManager";'
  );

  // Add streaming as a nav item
  if (!app.includes('"streaming"')) {
    app = app.replace(
  ...
  console.log('  UPDATED App.tsx (Stream tab)');
}
...
console.log('  New STREAM tab in the sidebar:');
console.log('    - Server, port, mount point, password config');
console.log('    - Bitrate selection (64k to 320k)');
console.log('    - Station name, genre, website');
console.log('    - One-click START STREAM / STOP STREAM');
console.log('    - Live uptime counter and bitrate display');
console.log('    - Stream URL shown for sharing with listeners');
console.log('    - Setup guide with licensing reminder');
console.log('');
console.log('  Captures audio directly from the playback engine');
console.log('  and sends to any Icecast-compatible server.');
console.log('');
console.log('  Push:');
console.log('    git add -A');
console.log('    git commit -m "v1.2.0 Icecast streaming"');
