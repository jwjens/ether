...
export default function FirstRunWizard({ onComplete }: Props) {
  ...
  return (
    <div style={{
      ...
      {/* Background glow */}
      <div style={{ position: "absolute", width: 700, height: 700, borderRadius: "50%", background: "radial-gradient(circle, rgb(from var(--accent-cyan) r g b / 0.05) 0%, transparent 70%)", pointerEvents: "none" }} />

      <div style={{ width: "100%", maxWidth: 640, position: "relative" as any }}>
        {/* Step dots */}
        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 48 }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width: i === step ? 24 : 8, height: 8, borderRadius: 0, background: i === step ? "var(--accent-cyan)" : i < step ? "rgb(from var(--accent-cyan) r g b / 0.4)" : "rgba(255,255,255,0.1)", transition: "all 0.3s ease" }} />
          ))}
        </div>
              ...
              {EXPERIENCE_MODES.map(mode => {
                ...
                return (
                  <button key={mode.id} onClick={() => setExperienceMode(mode.id)} style={{
                    ...
                    background: selected ? "rgb(from var(--accent-cyan) r g b / 0.08)" : "rgba(255,255,255,0.03)",
                    ...
                    <div style={{ width: 20, height: 20, borderRadius: "50%", border: `2px solid ${selected ? "var(--accent-cyan)" : "rgba(255,255,255,0.15)"}`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              {(["radio", "venue", "retail"] as VenueType[]).map((type) => {
                ...
                return (
                  <button key={type} onClick={() => setVenueType(type)} style={{
                    ...
                    background: selected ? "rgb(from var(--accent-cyan) r g b / 0.1)" : "rgba(255,255,255,0.03)",
                    border: `1.5px solid ${selected ? "var(--accent-cyan)" : "rgba(255,255,255,0.08)"}`,
          ...
          <div style={{ animation: "wiz-in 0.4s ease both", textAlign: "center" as any }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                margin: "0 auto",
                boxShadow: "0 0 40px rgb(from var(--accent-cyan) r g b / 0.4)",
      ...
      <style>{`
        ...
        @keyframes wiz-glow {
          0%,100%{box-shadow:0 0 40px rgb(from var(--accent-cyan) r g b / 0.4),0 0 80px rgba(167,139,250,0.2);}
          50%{box-shadow:0 0 60px rgb(from var(--accent-cyan) r g b / 0.6),0 0 100px rgba(167,139,250,0.35);}
        }
      `}</style>
    </div>
  );
