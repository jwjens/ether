...
export default function AutoCue({ onClose }: { onClose: () => void }) {
  ...
  const sourceLabel = (s: CueResult["source"]) => ({
    ...
    "id3-standard": { label: "ID3 Tag", color: "var(--accent-blue)" },
        ...
        <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border-primary)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
            {([
              ...
              <label key={val} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 12px", borderRadius: 0, border: `1px solid ${mode === val ? "var(--accent-cyan)" : "var(--border-primary)"}`, background: mode === val ? "rgb(from var(--accent-blue) r g b / 0.06)" : "var(--bg-secondary)", cursor: "pointer" }}>
          ...
          <div style={{ marginTop: 14, padding: "10px 12px", borderRadius: 0, background: "rgb(from var(--accent-blue) r g b / 0.06)", border: "1px solid rgb(from var(--accent-blue) r g b / 0.2)" }}>
      ...
      {status === "done" && (
        <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border-primary)", flexShrink: 0 }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            {id3Count > 0 && <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 0, background: "rgb(from var(--accent-blue) r g b / 0.15)", color: "var(--accent-cyan)" }}>✓ {id3Count} ID3 tags</span>}
