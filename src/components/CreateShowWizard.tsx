...
const CAT_COLORS = ["var(--accent-blue)","#a78bfa","#34d399","#f87171","#fbbf24","#fb923c","#e879f9","var(--accent-cyan)"];
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            ...
            background: i < step ? "var(--accent-blue)" : i === step ? "rgb(from var(--accent-blue) r g b / 0.2)" : "var(--bg-tertiary)",
...
export default function CreateShowWizard({ onClose, onDone }: Props) {
  ...
  const [showColor, setShowColor]     = useState("var(--accent-blue)");
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 180, overflowY: "auto" }}>
                    {clocks.map(c => (
                      <button key={c.id} onClick={() => setSelectedClockId(c.id)} style={{
                        ...
                        background: selectedClockId === c.id ? "rgb(from var(--accent-blue) r g b / 0.12)" : "var(--bg-tertiary)",
                        border: `1px solid ${selectedClockId === c.id ? "rgb(from var(--accent-blue) r g b / 0.4)" : "var(--border-primary)"}`,
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {categories.map(c => (
                      <button key={c.id} onClick={() => setSelectedCatId(selectedCatId === c.id ? null : c.id)} style={{
                        ...
                        background: selectedCatId === c.id ? (c.color ? c.color + "22" : "rgb(from var(--accent-blue) r g b / 0.12)") : "var(--bg-tertiary)",
                        border: `1px solid ${selectedCatId === c.id ? (c.color || "rgb(from var(--accent-blue) r g b / 0.4)") : "var(--border-primary)"}`,
