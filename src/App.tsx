...
export default function App() {
  ...
  // Log plays to database + preload B and C when A starts
  useEffect(() => {
    ...
    <div className={"h-screen flex flex-col " + (darkMode ? "dark-theme bg-zinc-950 text-zinc-100" : "")} style={{ background: "var(--bg-primary)", color: "var(--text-primary)" }}>
      ...
      <header style={{ height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border-primary)", boxShadow: "var(--shadow-sm)", flexShrink: 0 }}>
        <div className="flex items-center gap-0">
          ...
          <span className="ether-logo" style={{ marginLeft: 24, fontSize: 24 }}><span className="eth">Eth</span><span className="er">er</span></span>
          <span style={{ fontSize: 13, fontWeight: 400, color: "var(--text-tertiary)", marginLeft: 16 }}>{stationName}</span>
          <span style={{ fontSize: 10, color: "var(--text-tertiary)", fontWeight: 300, letterSpacing: "0.02em", marginLeft: 6 }}>v1.5</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-zinc-400">
          ...
          <ClockDisplay />
          <div style={{
            marginLeft: 12, padding: "6px 16px", borderRadius: 8, fontSize: 11, fontWeight: 800,
            letterSpacing: "0.12em", textTransform: "uppercase", cursor: "default", transition: "all 0.3s ease",
...
function DeckCard({ deck, deckId, accentColor, waveColor, playedColor }: { deck: DeckState | null; deckId: "A" | "B"; accentColor: string; waveColor: string; playedColor: string }) {
  ...
  const handlePlay = () => { if (!d) return; if (st === "playing") d.pause(); else if (st === "paused") d.resume(); else d.play(); };
...
function LibraryPanel({ onLoadA, onLoadB, onQueue }: { onLoadA: (s: SongRow) => void; onLoadB: (s: SongRow) => void; onQueue: (s: SongRow) => void }) {
  ...
  const analyzeLufs = async () => {
    ...
    for (const song of songs) {
      try { const gain = await invoke<number>("analyze_lufs", { filePath: song.file_path }); await execute("UPDATE songs SET gain_db=? WHERE id=?", [gain, song.id]); } catch (e) { console.error("LUFS error:", e); }
      done++;
  ...
  const relocateLibrary = async () => {
    ...
    setStatus('Relocated ' + fixed + ' songs to ' + newBase); setTimeout(() => setStatus(''), 4000); load();
  };
  const queueAll = () => { const items = filtered.filter(s => s.file_path); engine.addToQueue(items.map(s => ({ filePath: s.file_path!, title: s.title, artist: s.artist_name || "" }))); };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={relocateLibrary} style={{ padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "var(--bg-tertiary)", color: "var(--text-tertiary)", border: "1px solid var(--border-primary)", cursor: "pointer" }} title="Fix broken file paths">📁 Relocate</button>
          <button onClick={analyzeLufs} style={{ padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "var(--bg-tertiary)", color: "var(--text-tertiary)", border: "1px solid var(--border-primary)", cursor: "pointer" }} title="Analyze loudness">🎚 Normalize</button>
        </div>
      </div>
      <div className="flex items-center gap-2">
        ...
        <select onChange={async (e) => { if (!e.target.value) return; const catId = catList.find(c => c.code === e.target.value)?.id || null; const ids = filtered.map(s => s.id); for (const id of ids) { await execute("UPDATE songs SET category_id=? WHERE id=?", [catId, id]); } e.target.value = ""; load(); }} className="px-2 py-1.5 bg-zinc-800 border border-zinc-700 rounded text-xs text-zinc-300">
            ...
            <tbody>{filtered.map((s, i) => (
              <tr key={s.id} className="border-b border-zinc-800 hover:bg-zinc-800 group">
                ...
                <td className="px-2 py-1.5">
                  <select value={s.category_code || ""} onChange={async (e) => { const catId = catList.find(c => c.code === e.target.value)?.id || null; await execute("UPDATE songs SET category_id=? WHERE id=?", [catId, s.id]); load(); }} className="bg-zinc-800 border border-zinc-700 rounded text-[10px] text-zinc-200 px-1 py-0.5">
                    <option value="">—</option>
                ...
                <td className="px-2 py-1.5 text-right">
                  ...
                  <button onClick={async () => { if (confirm("Delete " + s.title + "?")) { await execute("DELETE FROM songs WHERE id=?", [s.id]); load(); } }} className="px-1.5 py-0.5 bg-zinc-800 hover:bg-red-900 rounded text-[9px] font-bold text-zinc-500 hover:text-red-400">X</button>
                </td>
              </tr>
            ))}</tbody>
          </table>
