'use strict';
// Pure sign-in provisioning decisions (Phase 3, subscription model). NO I/O — unit-testable in isolation
...
function selectAttachedStationsToMaterialize({ cloud = [], attachments = [], haveUuids = new Set(), tombstoned = new Set() } = {}) {
...
function chooseActiveStation({ localStations = [], attachments = [], hasActive = false } = {}) {
  if (hasActive) return null;
  const attached = new Set((attachments || []).map(a => a && a.station_uuid).filter(Boolean));
  return (localStations || []).find(s => s && attached.has(s.uuid)) || (localStations || [])[0] || null;
}

module.exports = { selectAttachedStationsToMaterialize, chooseActiveStation };
